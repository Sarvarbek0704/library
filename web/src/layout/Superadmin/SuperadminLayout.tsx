...
import {
  ...
  LogOut, Menu, X, BookMarked, ShieldCheck,
...
const superadminNav = [
  ...
  { to: '/superadmin/admin',         label: 'Adminlar',         icon: ShieldCheck },
  { to: '/superadmin/books',         label: 'Kitoblar',         icon: BookOpen },
  { to: '/superadmin/authors',       label: 'Mualliflar',       icon: PenSquare },
  { to: '/superadmin/categories',    label: 'Kategoriyalar',    icon: Tag },
  { to: '/superadmin/libraries',     label: 'Kutubxonalar',     icon: Building2 },
  { to: '/superadmin/users',         label: 'Foydalanuvchilar', icon: Users },
  { to: '/superadmin/members',       label: "A'zolar",          icon: UserCheck },
  { to: '/superadmin/memberships',   label: "A'zoliklar",       icon: BookMarked },
  { to: '/superadmin/payments',      label: "To'lovlar",        icon: CreditCard },
  { to: '/superadmin/member-stats',  label: 'Holat',            icon: BarChart3 },
  { to: '/superadmin/return_requests', label: 'Qaytarish',      icon: RotateCcw },
]
    ...
    <div className="flex min-h-screen bg-app">
      ...
      <aside className={cn(
        ...
        <div className="border-t border-[rgb(var(--border-subtle))] p-2 space-y-1 shrink-0">
          {collapsed ? (
            <div title={user?.firstName} className="flex justify-center py-2">
            ...
            <div className="flex items-center gap-2.5 rounded-lg px-2.5 py-2">
              ...
              <div className="flex-1 min-w-0">
                <p className="truncate text-sm font-medium text-[rgb(var(--text))]">{user?.firstName ?? 'SuperAdmin'}</p>
                <p className="text-xs text-[rgb(var(--text-muted))]">Super Admin</p>
              </div>
      ...
      {/* Main — min-w-0 prevents flex child from expanding past viewport */}
